...
public class Cga //manipulates the species, genomes and innovations
{
    ...
    { 
        ...
        CInnovation.CreateNewInnovation(-1, -1, "neuron", inputs+1, "output"); //adds innovation

        //link innovations

        for(int i = 0; i < inputs; i++)
        {
            CInnovation.CreateNewInnovation(i + 1, inputs+1, "link", -1, "none");
        }

        //creates the birds and their neural networks
    ...
    public void Epoch(List<double> FitnessScores)
    {
        ...
        for(int spc = 0; spc < vecSpecies.Count; spc++)
        {
            ...
            if(NumSpawnedSoFar < GameManager.NUM_BIRDS)
            {
                ...
                while(NumToSpawn > 0)
                {
                    ...
                    } else
                    {
                        ...
                        } else
                        {
                            ...
                            if (Random.Range((float)0, (float)1) < GameManager.CROSSOVER_RATE)
                            {
        ...
        if(NumSpawnedSoFar < GameManager.NUM_BIRDS)
        {
            ...
            int Rqd = GameManager.NUM_BIRDS - NumSpawnedSoFar;

            while(Rqd > 0)
            {
                newPopulation.Add(TournamentSelection(GameManager.NUM_BIRDS / 2));
                Rqd--;
            }
        }
    ...
    public double UpdateMember(int index, List<double> inputs) //updates the AI
    {
        ...
        double response = outputs[0];
        return response; //has to greater than this to jump
    }
