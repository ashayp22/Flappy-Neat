...
public class CGenome
{
    ...
    public CGenome(int id, int inputs, int output)
    {
        ...
        //one output
        vecNeurons.Add(new SNeuronGene(inputs+1, 1, (double)(numInputs-1)/2, false, "output"));

        //now create the links between them
        for(int i = 0; i < numInputs; i++)
        {
            int innovation = CInnovation.CheckInnovation(i + 1, inputs + 1, "link");
            vecLinks.Add(new SLinkGene(i + 1, inputs + 1, true, false, Random.Range(0.01f, 1), innovation)); //adds to vector
    ...
    public void AddLink(double MutationRate, double ChanceOfLooped, int NumTrysToFindLoop, int NumTrysToAddLink) //adds a link
    {
        ...
        if (Random.Range(0, 1) > MutationRate) return; //returns dependent on mut rate
        ...
        if(Random.Range(0, 1) < ChanceOfLooped)
        {
        ...
        } else
        {
            ...
            {
                ID_neuron1 = vecNeurons[Random.Range(0, vecNeurons.Count-1)].getID();
                ID_neuron2 = vecNeurons[Random.Range(numInputs+1, vecNeurons.Count - 1)].getID();
        ...
        //is this link recurrent(the y pos will be higher for the first)
        ...
        if(id < 0) //create a new innovation
        {
            ...
            SLinkGene newGene = new SLinkGene(ID_neuron1, ID_neuron2, true, recurrent, Random.Range(-1, 1), linkId);
        ...
        } else
        {
            ...
            SLinkGene newGene = new SLinkGene(ID_neuron1, ID_neuron2, true, recurrent, Random.Range(-1, 1), id);
    ...
    public void MutateWeights(double mut_rate, double prob_new_mut, double MaxPertubation) //mutates the connection weights
    {
        ...
        {
            if((Random.RandomRange(0, 100) / 10) < mut_rate)
            {
                if((Random.RandomRange(0, 100) / 10) < prob_new_mut) //changes the weight
                {
                    link.changeWeight(Random.Range(0.01f, 1));
                } else
                {
